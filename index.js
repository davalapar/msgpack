...
const tempEncodeBuffer = Buffer.allocUnsafe(2 ** 12).fill(0);
...
const internalDecode = () => {
  ...
  if (tempDecodeBuffer[tempDecodeBufferOffset] < 192) {
    ...
    if (tempDecodeBuffer[tempDecodeBufferOffset] < 144) { // fixmap
      length = tempDecodeBuffer[tempDecodeBufferOffset] & 31;
      value = {};
      tempDecodeBufferOffset += 1;
      if (dictionaryEnabled === true) {
        for (let i = 0, key; i < length; i += 1) {
          key = internalDecode();
    ...
    if (tempDecodeBuffer[tempDecodeBufferOffset] < 160) { // fixarray
  ...
  if (tempDecodeBuffer[tempDecodeBufferOffset] > 223) { // negative fixint
  ...
  switch (tempDecodeBuffer[tempDecodeBufferOffset]) {
    ...
    case 206: // uint 32
      ...
      return value;
    case 207: // uint 64
      value = (tempDecodeBuffer.readUInt32BE(tempDecodeBufferOffset += 1) * (2 ** 32)) + tempDecodeBuffer.readUInt32BE(tempDecodeBufferOffset += 4);
      tempDecodeBufferOffset += 4;
      return value;
    case 208: // int 8
    ...
    case 210: // int 32
      ...
      return value;
    case 211: // int 64
      value = (tempDecodeBuffer.readInt32BE(tempDecodeBufferOffset += 1) * (2 ** 32)) + tempDecodeBuffer.readUInt32BE(tempDecodeBufferOffset += 4);
      tempDecodeBufferOffset += 4;
      return value;
...
module.exports = { encode, decode, register };
